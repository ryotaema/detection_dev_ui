...
#
#   自作モデルと違い、重みは学習の成果物ではなく **人が取ってくるもの** なので
#   models/ の直下ではなく .sam3/ に分けて置く（モデル一覧に混ざらないように）。
#   3.45GB あるためイメージには焼き込まず、関数コンテナへマウントして使う。
#   取得には https://huggingface.co/facebook/sam3 でのアクセス承認が要る。
# ---------------------------------------------------------------------------
SAM3_DIR          = MODELS_DIR / ".sam3"
SAM3_WEIGHTS_NAME = "sam3.pt"
SAM3_MOUNT_PATH   = "/opt/nuclio/sam3"          # 関数コンテナ側のマウント先

# 重みを置く場所は、置く前から開けるようにしておく（案内先が無いと迷うため）
